...
public class RightClickMeButton {
	...
	try {
		...
		String buttons = p.getProperty("buttons");	
        String scroll = p.getProperty("scroll");	
        String scroll1 = p.getProperty("scroll1");	
        String rightClickMeButton = p.getProperty("rightClickMeButton");	
		...
		d.findElement(By.xpath(buttons)).click();
		Thread.sleep(3000);

		//click on right click me
		
		WebElement tar = d.findElement(By.xpath(rightClickMeButton));
		
		Actions act = new Actions(d);
		
		act.moveToElement(tar).contextClick().perform();
		Thread.sleep(3000);
